...
const ProjectCard = (props: Project) => {
  ...
  return (
    <div className="pt-6 border-t-[1px] mb-0 border-t-[#f5efff]/20 border-opacity-20">
      <div
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        className="h-96 w-full relative p-6 rounded-2xl overflow-hidden flex flex-row hover:shadow-2xl transition-all duration-300 ease-out hover:shadow-violet-500/[0.1]"
      >
        <div
          className={`absolute w-full h-full top-0 left-0 bg-white transition-opacity ease-out duration-300 ${
            hovered ? " opacity-10 " : "opacity-0"
          }`}
        ></div>
        <img src={props.image} className="rounded-xl z-20" alt={props.title} />
